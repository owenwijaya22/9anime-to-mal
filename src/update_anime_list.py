...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            if task == 'd':
                tasks =  [delete_list(access_token, anime_id, session, status_list) for anime_id in anime_ids]
            elif task == 'u':
                tasks =  [update_list(access_token, anime_id, session, status_list) for anime_id in anime_ids]
            r = await asyncio.gather(*tasks)
            print(status_list)
            if 500 in status_list:
                print("Mal's server experienced server error,  please run the program one more time :D")
